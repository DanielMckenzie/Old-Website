...
% October 23 2017
% This test version 3, a faster version of the original function, version 1
%
...
%Run and time Compressive Clustering.
 tic
 C1C = SCPv3(A,n0, index1);
 C1comp = setdiff(1:n,C1C);
 SCPv3Time = toc;
 disp(['Time taken by SCPv3 ', num2str(SCPv3Time)])
 newIndsC = [C1C, C1comp];
 A_newC = A(newIndsC, newIndsC);
 I3 = mat2gray(full(A_newC));
 figure
 imshow(imcomplement(I3));
 title('Result of appliyng SCP to the scrambled matrix. Cluster 1 is clearly visible')
 clear newIndsC A_newC
 
% Computing the error
ErrorC = length(setdiff(Cluster1True,C1C'));
MissedC = length(setdiff(C1C',Cluster1True));
disp(['Number of indices misclassified by SCPv3 ', num2str(MissedC)])
disp(['Number of indices missed by SCPv3 ', num2str(ErrorC)])
